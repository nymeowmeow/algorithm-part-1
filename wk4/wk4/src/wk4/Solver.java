...
public class Solver {
	...
	List<Board> minmoves = new ArrayList<Board>();
    ...
    public Solver(Board initial) {
    	...
    	Comparator<SearchNode> comp = new Comparator<SearchNode>() {
    		...
    		{
    			return first.getCurrent().hamming() + first.getStep()
    					- second.getCurrent().hamming() - second.getStep();
    		}
    	};
    ...
    private void addNeighbors(SearchNode current, MinPQ<SearchNode> queue)
    {
    	...
    	while (it.hasNext()) {
    		...
    		{
    			if (!prev.getCurrent().equals(board))
    			{
